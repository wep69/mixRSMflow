...
**Purpose:** define the final documentation hierarchy, file placement,
numbering, build strategy, cross-references, topic ownership, and
maintenance rules for the package vignettes and extended instructional
monographs.
...
The final documentation should use **three complementary layers** rather
than turning every package vignette into a long monograph.

### Layer A. Focused package vignettes

The existing `01` to `22` `.Rmd` files remain concise, buildable package
vignettes. They provide quick access to one concept or workflow and
should be the first destination for a user searching for a specific
package feature.

### Layer B. Extended instructional monographs

The master tutorial plus Extended Vignettes `24` to `33` provide
long-form teaching material. They are intentionally supplied as Markdown
source and should remain topic-owned documents rather than repeating the
complete package workflow in every file.

### Layer C. Function reference

The Rd documentation remains the authoritative function-level API
reference. A vignette should explain *why* and *when* to use a function;
the manual should define its formal arguments and return object
precisely.
...
This is the main **start-to-finish teaching tutorial**. It owns the
complete package workflow and is the only extended document allowed to
revisit all major modules in sequence.
...
Do not create another extended tutorial unless it owns a genuinely
distinct scientific domain that cannot be handled as a subsection or
focused vignette.
...
The safest structure for package build time and optional dependencies
is:
...
The long tutorials contain many code examples, including optional
Bayesian, mixed-model, Plotly, Shiny, and Gaussian-process workflows.
Requiring every long monograph to execute during every `R CMD check`
would:
...
- duplicate tests already handled by the package test and validation
  system.

The extended Markdown files should therefore be treated as **installed
instructional sources**. Formal R vignette wrappers can expose them
without executing all code blocks.

------------------------------------------------------------------------

## 4. Lightweight Rmd wrapper strategy

For an extended Markdown file, create a small formal R vignette whose
only job is to include the Markdown source as rendered text.
...
Before adopting this wrapper pattern permanently, test it under the real
package build because working directories can differ among build tools.
If relative-path inclusion proves fragile, copy the Markdown into the
vignette build directory during a documented pre-build step or convert
the long source to an `.Rmd` with all long examples nonexecuting.

The key objective is that the **long code examples are displayed but not
automatically executed as part of every package check**.
...
Keep them as explanatory Markdown source. Code is intended for the
reader to run locally. Do not make routine CRAN build success depend on:
...
The following matrix is the principal mechanism used to prevent overlap
among the extended monographs.
...
When a topic must appear outside its owning vignette, use a short
cross-reference rather than repeating the full explanation.

Example:

> “For the full mathematical treatment of FDS and VDG, see Extended
> Vignette 25.”
...
The focused files should end with a short “Continue with the extended
tutorial” link once the final package paths are established.
...
The numeric filenames remain `28`, `29`, `30`, `31`, etc., but the
pedagogical reading order may place `31` before `28` when the user’s
analysis first requires advanced stochastic response models.
...
Each function should have one **primary extended home** even when it
appears in several workflows.
...
The master tutorial can call all of them, but it should not replace
their thematic monographs.
...
Do not copy this block blindly until the final wrapper filenames exist.
`pkgdown` article keys must match actual article sources.
...
but routine package checks should not require every long example to
execute.
...
or use nonexecuting examples where runtime or external compilation is
excessive.

Backend functionality itself belongs in automated tests and the
validation matrix.
...
- [`mix_bo()`](https://wep69.github.io/mixRSMflow/reference/mix_bo.md)
  if the optional GP backend is absent;
- Plotly interaction;
- Shiny launch;
- benchmark loops;
...
This preserves truthful examples without turning documentation into the
...
The extended Markdown files should normally show **code without frozen
numerical results** unless those results are part of a validated
golden/frozen teaching dataset.

Do not invent coefficient tables, p-values, optimum coordinates, or
benchmark timings.

When a teaching result is simulated, label it explicitly as simulated.

When a result must be reproduced locally, state that the code generates
it rather than writing an unverified number into the prose.
...
The full master tutorial may define notation globally. Thematic
monographs should redefine only what is needed locally.
...
Other tutorials may generate only figures required to interpret their
own method.
...
This prevents every vignette from reproducing the same ternary contour
section.
...
Create lightweight `.Rmd` wrappers only if the long documents are to
appear as formal package vignettes/pkgdown articles.
...
The extended docs contain many examples. API signatures must be
re-audited after any future function renaming.
...
Do not manually update only one copy of an example and leave stale calls
elsewhere.
...
This hierarchy gives the package both **fast reference documentation**
and **textbook-like instructional depth** without making users read the
...
expose extended tutorials through lightweight wrappers/pkgdown if
desired;
...
The final `mixRSMflow` documentation should **not** consist of 33
equally long compiled vignettes.
...
This preserves pedagogical depth, avoids subject overlap, keeps package
build times manageable, and makes the documentation useful to beginners,
advanced analysts, reviewers, and package developers at the same time.
